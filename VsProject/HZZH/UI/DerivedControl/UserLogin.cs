    ...
    public partial class UserLogin: Form
    {
        ...
        private void btn_Login_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (ConfigHandle.Instance.UserDefine.UserList[i].Name.Equals(Acountlistcbx.Text) && ConfigHandle.Instance.UserDefine.UserList[i].PassWord.Equals(GetSHA1HashData(txt_Password.Text)))
                    {
        ...
        private void UserLogin_Load(object sender, EventArgs e)
        {
            for (int i = 0; i < ConfigHandle.Instance.UserDefine.UserList.Count; i++)
            {
                ...
                {
                    Acountlistcbx.Items.Add(ConfigHandle.Instance.UserDefine.UserList[i].Name);
                }
            }
            if (Acountlistcbx.Items.Count > 0)
            {
                Acountlistcbx.SelectedIndex = 0;
            }
        }
