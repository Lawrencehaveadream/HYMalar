...
namespace HZZH.UI.DerivedControl
{
    public partial class Frm_MotorParam : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (axis != null)
            {
                label1.Text = axis.currPos.ToString("0.00");
                //label2.Text = axis.errMesg;
                label11.Text = axis.AxisName + ":" + axis.errMesg;
                if (axis.errCode == 0)
                {
                ...
                Lb_NegLimt.Text = axis.I_NegLimt.XIndex;
                Lb_NegLimt.BackColor = axis.I_NegLimt.Value?Color.LightGreen:Color.Gray;

                Lb_posLimt.Text = axis.I_PosLimt.XIndex;
                Lb_posLimt.BackColor = axis.I_PosLimt.Value ? Color.LightGreen : Color.Gray;

                Lb_Orig.Text = axis.I_Orgin.XIndex;
                Lb_Orig.BackColor = axis.I_Orgin.Value ? Color.LightGreen : Color.Gray;

                if(axis.LimitMode == 3 || axis.LimitMode == 2)
                {

                    Lb_NegLimt.Visible = true;
                    Lb_posLimt.Visible = true;
                }
                else
                {
                    Lb_NegLimt.Visible = false;
                    Lb_posLimt.Visible = false;
                }
                ...
                Lb_MinVal.Text = axis.SoftMinLimit.ToString();
                Lb_MaxVal.Text = axis.SoftMaxLimit.ToString();



                Stb_AxisCurrpos.Minimum = (int)axis.SoftMinLimit;
                Stb_AxisCurrpos.Maximum = (int)axis.SoftMaxLimit;
                if((int)axis.currPos>Stb_AxisCurrpos.Minimum && (int)axis.currPos < Stb_AxisCurrpos.Maximum)
                {
                    Stb_AxisCurrpos.Value = (int)axis.currPos;
                }
                else if((int)axis.currPos <= Stb_AxisCurrpos.Minimum)
                {
                    Stb_AxisCurrpos.Value = Stb_AxisCurrpos.Minimum;
                }
                else if ((int)axis.currPos >= Stb_AxisCurrpos.Maximum)
                {
                    Stb_AxisCurrpos.Value = Stb_AxisCurrpos.Maximum;
                }
            }
